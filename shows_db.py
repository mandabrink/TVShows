...
class showsDB:
    ...
    def createUser(self, fname, lname, username, pass_word):
        data = [fname, lname, username, pass_word]
        self.cursor.execute("INSERT INTO users (fname, lname, username, pass) VALUES (?, ?, ?, ?)", data)
        self.connection.commit()

    def getUser(self, username, pass_word):
        date = [username, pass_word]
        self.cursor.execute("SELECT username, pass FROM users WHERE username = ? and pass = ?", date)